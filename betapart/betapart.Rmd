...
bp_tidy <- function(bp_list) {
  a <- bp_list$beta.jac |> tidy(diagonal = F, upper = F) |> mutate(Jaccard = distance, distance = NULL)
  b <- bp_list$beta.jne |> tidy(diagonal = F, upper = F) |> mutate(Nestedness = distance, distance = NULL)
  c <- bp_list$beta.jtu |> tidy(diagonal = F, upper = F) |> mutate(Turnover = distance, distance = NULL)
...
ftab_b_bpt |>
  ...
  # within group only
  filter(autoFmtGroup1 == autoFmtGroup2) |>
  glimpse() # 7535

# Backwards, starting from metadata -> all pairs -> just pairs within the group
meta_bp_autoFmtGroup <- meta |> 
  inner_join(meta, by = "autoFmtGroup", relationship = "many-to-many") |> 
  mutate(item1 = SampleID.x, item2 = SampleID.y, autoFmtGroup, .keep = "none") |>
  filter(item1 != item2) |>
  # we avoid double counting by using only matching distances from this object
  inner_join(ftab_b_bpt)
```

```{r bp_dispersion_plot}
meta_bp_autoFmtGroup |>
  ...
  ggboxplot( "autoFmtGroup", "value", color = "Part") +
  labs(y = "Pairwise distance to all \nother samples within this group") +
...
meta_bp_autoFmtGroup |>
  ...
  labs(x = "Sample", y = "Pairwise distance to all \nother samples within this group") +
